// https://stackoverflow.com/questions/34866921/partial-permutations
...
void rotate_discontinuous3(
    ...
    if (d1 <= d2)
        rotate_discontinuous(std::next(first2, d2 - d1), last2, d1, first3,
                             last3, d3);
    else {
        rotate_discontinuous(std::next(first1, d2), last1, d1 - d2, first3,
                             last3, d3);
...
bool combine_discontinuous3_(
    ...
    if (d1 == 1) {
        ...
        BidirIter i2 = first2;
        for (D d22 = d2; i2 != last2; ++i2, --d22) {
            if (combine_discontinuous3_(f1p, last1, d1 - 1, i2, last2, d22,
                                        first3, last3, d3, f, d + 1))
                return true;
            swap(*first1, *i2);
        }
        i2 = first3;
        for (D d22 = d3; i2 != last3; ++i2, --d22) {
            if (combine_discontinuous(f1p, last1, d1 - 1, i2, last3, d22, f,
                                      d + 1))
                return true;
            swap(*first1, *i2);
        }
    ...
    if (d != 0) {
        if (d2 > 1)
            rotate_discontinuous3(first1, last1, d1, std::next(first2), last2,
                                  d2 - 1, first3, last3, d3);
        else
            rotate_discontinuous(first1, last1, d1, first3, last3, d3);
    } else
        rotate_discontinuous3(first1, last1, d1, first2, last2, d2, first3,
                              last3, d3);
    return false;
}
...
bool combine_discontinuous3(
    ...
    return combine_discontinuous3_(first1, last1, d1, first2, last2, d2, first3,
                                   last3, d3, fbc);
}
...
template <class BidirIter>
std::uintmax_t count_each_combination(BidirIter first, BidirIter mid,
                                      BidirIter last) {
...
//    thusly.
template <class BidirIter, class Function>
Function for_each_permutation(BidirIter first, BidirIter mid, BidirIter last,
                              Function f) {
    typedef typename std::iterator_traits<BidirIter>::difference_type D;
    ...
    detail::combine_discontinuous(first, mid, d1, mid, last,
                                  std::distance(mid, last), pf);
    return std::move(f);
}
...
template <class BidirIter>
std::uintmax_t count_each_permutation(BidirIter first, BidirIter mid,
                                      BidirIter last) {
...
template <class BidirIter, class Function>
Function for_each_circular_permutation(BidirIter first, BidirIter mid,
                                       BidirIter last, Function f) {
    for_each_combination(first, mid, last,
                         detail::circular_permutation<Function&, BidirIter>(
    ...
    if (d1 <= d2) {
        try {
            ...
            throw std::overflow_error(
                "overflow in count_each_circular_permutation");
        }
        for (--d1; d1 > 1; --d1) {
...
template <class BidirIter>
std::uintmax_t count_each_circular_permutation(BidirIter first, BidirIter mid,
                                               BidirIter last) {
...
class rev2 {
    ...
    public:
    rev2(Function f, BidirIter first1, BidirIter last1, D d1, BidirIter first2,
         BidirIter last2, D d2)
        : f_(f),
          first1_(first1),
          last1_(last1),
          d1_(d1),
...
class rev3 {
    ...
    public:
    rev3(Function f, BidirIter first1, BidirIter last1, D d1, BidirIter first2,
         BidirIter last2, D d2, BidirIter first3, BidirIter last3, D d3)
        : f_(f),
          first1_(first1),
          last1_(last1),
          d1_(d1),
    ...
    bool operator()() {
        rev2<Function, BidirIter> f(f_, first2_, last2_, d2_, first3_, last3_,
                                    d3_);
        return permute(first1_, last1_, d1_, f);
    }
};
...
bool reversible_permutation<Function, Size>::operator()(BidirIter first,
                                                        BidirIter last) {
    typedef typename std::iterator_traits<BidirIter>::difference_type
        difference_type;
    ...
    // else if the range has length 3, we need one more call and the fix is
    // easy.
    else if (s_ == 3) {
        swap(*am1, *a);
    ...
    //     the middle.
        ...
        //     [first+1, middle) + [middle+1, last).  Run through all
        //     permutations of that discontinuous range.
        swap(*am1, *a);
        BidirIter b = first;
        ...
        if (combine_discontinuous(bp1, a, s2 - 1, ap1, last, s_ - s2 - 1, f2))
            return true;
        // Swap the current first element into every place from first+1 to
        // middle-1.
        //   For each location, hold it steady to create the following
        //   discontinuous range (made of 3 ranges): [first, b-1) + [b+1,
        //   middle) + [middle+1, last). For each b in [first+1, middle-1), run
        //   through all permutations of
        ...
        for (Size i = 1; i < s2 - 1; ++i, ++bm1, ++b, ++bp1) {
            ...
            if (combine_discontinuous3(first, b, i, bp1, a, s2 - i - 1, ap1,
                                       last, s_ - s2 - 1, f3))
                return true;
        }
        // swap b into into middle-1, creates a discontinuous range:
        //     [first, middle-1) + [middle+1, last).  Run through all
        //     permutations of that discontinuous range.
        swap(*bm1, *b);
        F2 f21(f, first, b, s2 - 1, ap1, last, s_ - s2 - 1);
        if (combine_discontinuous(first, b, s2 - 1, ap1, last, s_ - s2 - 1,
                                  f21))
            return true;
...
template <class BidirIter, class Function>
Function for_each_reversible_permutation(BidirIter first, BidirIter mid,
                                         BidirIter last, Function f) {
    typedef typename std::iterator_traits<BidirIter>::difference_type D;
    for_each_combination(first, mid, last,
...
UInt count_each_reversible_permutation(UInt d1, UInt d2) {
    ...
    if (d1 > std::numeric_limits<UInt>::max() - d2)
        throw std::overflow_error(
            "overflow in count_each_reversible_permutation");
    UInt n = d1 + d2;
    UInt r = 1;
    if (d1 > 1) {
        r = n;
...
template <class BidirIter>
std::uintmax_t count_each_reversible_permutation(BidirIter first, BidirIter mid,
                                                 BidirIter last) {
...
class reverse_circular_permutation {
    ...
    bool operator()(BidirIter first, BidirIter last) {
        ...
        return reversible_permutation<BoundFunc, D>(f, std::distance(n, last))(
            n, last);
    }
};
...
Function for_each_reversible_circular_permutation(BidirIter first,
                                                  BidirIter mid, BidirIter last,
                                                  Function f) {
    for_each_combination(
        first, mid, last,
        detail::reverse_circular_permutation<Function&, BidirIter>(
            f, std::distance(first, mid)));
    return std::move(f);
}
...
UInt count_each_reversible_circular_permutation(UInt d1, UInt d2) {
    ...
    try {
        ...
        throw std::overflow_error(
            "overflow in count_each_reversible_circular_permutation");
    }
    if (d1 > 3) {
        for (--d1; d1 > 2; --d1) {
            if (r > std::numeric_limits<UInt>::max() / d1)
