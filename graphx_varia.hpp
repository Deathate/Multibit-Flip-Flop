...
public:
    ...
    void remove_vertex(std::ptrdiff_t vertex_id)
    {
        ...
        for (const auto& edge : m_edges)
        {
            if ((edge.source_vertex_id() == vertex_id) || (edge.target_vertex_id() == vertex_id))
            {
                ...
                m_vertices[edge.target_vertex_id()].remove_in_edge(edge.id());
    ...
    std::ptrdiff_t add_edge(std::ptrdiff_t from_vertex_id, std::ptrdiff_t to_vertex_id, const E& data)
    {
        ...
        m_vertices[to_vertex_id].add_in_edge(new_edge_id);
    ...
    std::set<std::ptrdiff_t> outgoings(std::ptrdiff_t vertex_id) const
    {
        assert(is_vertex_present(vertex_id));

        return m_vertices[vertex_id].edge_ids();
    }
    std::set<std::ptrdiff_t> incomings(std::ptrdiff_t vertex_id) const
    {
        assert(is_vertex_present(vertex_id));

        return m_vertices[vertex_id].edge_ids_in();
    }
...
private:
    ...
    public:
        ...
        void add_in_edge(std::ptrdiff_t edge_id)
        {
            in_edge_ids.insert(edge_id);
        }
        ...
        void remove_in_edge(std::ptrdiff_t edge_id)
        {
            in_edge_ids.erase(edge_id);
        }
        ...
        std::set<std::ptrdiff_t> edge_ids_in() const
        {
            return in_edge_ids;
        }
    ...
    private:
        ...
        std::set<std::ptrdiff_t> in_edge_ids;
        V m_data;
    };
