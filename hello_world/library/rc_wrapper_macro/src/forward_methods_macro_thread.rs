            ...
            if let Some(receiver) = method.sig.receiver() {
                ...
                let method_body = if !is_reference {
                    ...
                    quote! {
                        // #[inline(always)]
                        #(#method_attrs)*
                        #method_vis fn #method_name #method_generics (&self, #(#inputs),*) -> parking_lot::MappedMutexGuard<'_, #inner_ty> #method_where {
                            let guard = self.0.lock();
                            parking_lot::MutexGuard::map(guard, |a| &mut a.#method_name(#call_args))
                        }
                    }
                };
