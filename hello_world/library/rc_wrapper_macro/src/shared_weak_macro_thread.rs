...
pub fn expand(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let accessors = fields.iter().map(|f| {
        ...
        // For Copy primitives, return by value
        let getter_fn = if is_primitive_copy(&ty) {
            ...
            // For non-Copy, return a mapped guard into the field
            // parking_lot: MutexGuard::map -> MappedMutexGuard<'_, FieldTy>
            quote! {
                pub fn #getter(&self) -> parking_lot::MappedMutexGuard<'_, #ty> {
                    parking_lot::MutexGuard::map(self.borrow(), |inner| &mut inner.#name)
                }
            }
        };
        ...
        quote! {
            ...
            pub fn #getter_mut(&self) -> parking_lot::MappedMutexGuard<'_, #ty> {
                parking_lot::MutexGuard::map(self.borrow_mut(), |inner| &mut inner.#name)
            }
    ...
    let accessors_weak = fields.iter().map(|f| {
        ...
        let getter_fn = if is_primitive_copy(&ty) {
            quote! {
                pub fn #getter(&self) -> #ty {
                    self.0.upgrade().unwrap().lock().#name
                }
            }
        } else {
            quote! {}
        ...
        quote! {
            ...
            pub fn #setter(&self, value: #ty) -> &Self {
                self.0.upgrade().unwrap().lock().#name = value;
                self
            }
        }
    });
    ...
    let expanded = quote! {
        ...
        #(#attrs)*
        #vis struct #shared_name(pub std::sync::Arc<parking_lot::Mutex<#struct_name>>);
        ...
        impl #shared_name {
            ...
            /// Convenience: lock and get &mut to the whole value (via guard deref).
            pub fn borrow_mut(&self) -> parking_lot::MutexGuard<'_, #struct_name> {
                self.0.lock()
            }

            /// Alias for symmetry with previous API.
            pub fn borrow(&self) -> parking_lot::MutexGuard<'_, #struct_name> {
                self.0.lock()
            }

            pub fn get_ref(&self) -> &std::sync::Arc<parking_lot::Mutex<#struct_name>> {
                &self.0
            }
        ...
        impl std::fmt::Debug for #shared_name {
            fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                f.debug_struct(stringify!(#shared_name))
                    .field("inner", &"<Arc<parking_lot::Mutex<...>>>")
                    .finish()
            }
        }

        impl From<std::sync::Arc<parking_lot::Mutex<#struct_name>>> for #shared_name {
            fn from(inner: std::sync::Arc<parking_lot::Mutex<#struct_name>>) -> Self {
                Self(inner)
            }
        }
        ...
        impl From<#shared_name> for std::sync::Arc<parking_lot::Mutex<#struct_name>> {
        ...
        #vis struct #weak_name(pub std::sync::Weak<parking_lot::Mutex<#struct_name>>);

        impl #weak_name {
            pub fn new(value: &std::sync::Arc<parking_lot::Mutex<#struct_name>>) -> Self {
        ...
        impl From<std::sync::Weak<parking_lot::Mutex<#struct_name>>> for #weak_name {
            fn from(inner: std::sync::Weak<parking_lot::Mutex<#struct_name>>) -> Self {
                Self(inner)
            }
        }

        impl From<#weak_name> for std::sync::Weak<parking_lot::Mutex<#struct_name>> {
