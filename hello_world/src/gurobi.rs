...
pub fn solve_mutiple_knapsack_problem(
    ...
    knapsack_capacities : &Vec<i32>, ) -> Vec<Vec<usize>> {
    if items
        .is_empty() {
            return Vec::new ();
        }
    ...
    let gurobi_output : grb::Result<_> = crate::redirect_output_to_null(false, || {
        ...
        panic!("Optimization failed."); }).unwrap();
...
pub fn optimize_single_timing(
    mbffg : &mut MBFFG,
    insts : &Vec<&SharedInst>, ) -> grb::Result<(float, float)> {
    let mut model = redirect_output_to_null(true, || {
        let env = Env::new("")?;
        let mut model = Model::with_env("", env)?;
        model.set_param(param::LogToConsole, 0)?;
        Ok::<_, grb::Error>(model) }).unwrap().unwrap();
    ...
    let get_position = | pin : &SharedPhysicalPin |->(Expr, Expr) {
        if optimized_cell_ids
            .contains(&pin.get_gid()) {
    ...
    for
        ...
        if !fixed_record
            .is_empty() {
                let max_fixed_record = fixed_record
                                           ...
                                           .unwrap();
                max_fixed_delay = max_fixed_record.calculate_total_delay(displacement_delay) - ff_d_dist * displacement_delay;
            }
        let mut vars = Vec::new ();
        for
            record in dynamic_records.iter() {
                ...
                let unchanged_delay = record.calculate_total_delay(displacement_delay) - (record.ff_q_dist() + ff_d_dist) * displacement_delay;
                model
                    .add_constr(
                        "",
        ...
        if let
            Some(ff_d) = max_record.ff_d.as_ref() {
                let ff_d_expr =
...
pub fn optimize_multiple_timing(
    mbffg : &mut MBFFG,
    insts : &[&SharedInst],
    simplified_ratio : float, ) -> grb::Result<Dict<usize, (float, float)>> {
    let mut model = redirect_output_to_null(true, || {
        let env = Env::new("")?;
    ...
    let get_position = | pin : &SharedPhysicalPin |->(Expr, Expr) {
        if let
            Some(cell) = x_var.get(&pin.get_gid()) {
                let(x, y) = pin.relative_pos();
                (cell.x + x, cell.y + y)
            }
        else {
    ...
    for
        ...
        let max_fixed_delay = fixed_record
                                  .iter()
                                  .max_by_key(| record | OrderedFloat(record.calculate_total_delay(displacement_delay)))
                                  .map(| record | {record.calculate_total_delay(displacement_delay) - ff_d.ff_d_dist() * displacement_delay})
                                  .unwrap_or(0.0);
        let mut vars = Vec::new ();
        for
            record in dynamic_records.iter() {
                ...
                let unchanged_delay = record.calculate_total_delay(displacement_delay) - (record.ff_q_dist() + ff_d.ff_d_dist()) * displacement_delay;
                model
                    .add_constr(
                        "",
                        c !(var >= unchanged_delay + ff_q_expr * displacement_delay), )
...
//     cover_map : &Vec<Vec<bool>>,
//     tile_size : (uint, uint), ) -> grb::Result<Vec<Vec<bool>>> {
//     let(num_row, num_column) = (cover_map.len(), cover_map[0].len());
//     let(tile_h, tile_w) = cast_tuple::<_, usize>(tile_size);
...
//     for
//         i in 0.. = num_column - tile_w {
//         for
//             j in 0.. = num_row - tile_h {
...
//     model.set_objective(x.iter().flatten().grb_sum(), Maximize) ? ;
...
//             (0..num_row).step_by(tile_h).for_each(| i | {(0..num_column).step_by(tile_w).for_each(| j | {
//                 for
//                     r in i..max(i + tile_h, num_row) {
//                     for
//                         c in j..max(j + tile_w, num_column) {
//                             if model
//                                 .get_obj_attr(attr::X, &x[c][r]).unwrap() > 0.1 {
//                                     result[i][j] = true;
//                                     break;
//                                 }
//                         }
