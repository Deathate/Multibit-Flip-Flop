...
def color_map(n):
    if not hasattr(color_map, "colors"):
        from pypalettes import get_hex, get_rgb, load_cmap

        colors = list(
            chain.from_iterable(
                [load_cmap(x).rgb for x in ["Acanthurus_olivaceus", "Signac", "Antique"]]
            )
        )
        colors = [
            c
            ...
            if i
            in [0, 1, 3, 4, 7, 8, 10, 12, 13, 15, 17, 18, 19, 20, 21, 22, 23, 25, 28, 29, 30, 31]
        ]
