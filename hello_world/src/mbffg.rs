...
pub fn cal_max_record(records : &[PrevFFRecord], displacement_delay : float) -> &PrevFFRecord{
    records
        ...
        .unwrap()} fn cal_mean_dis_to_center(group : &[&SharedInst])
    ->float {
...
pub struct MBFFG {
                                                                                                ...
                                                                                                current_insts : Dict<String, SharedInst>,
                                                                                                                disposed_insts : Vec<SharedInst>,
                                                                                                                                 pub prev_ffs_cache : Dict<PinId, Set<PrevFFRecord>>,
                                                                                                                                                      pub prev_ffs_query_cache : Dict<PinId, (PrevFFRecord, Dict<SharedPhysicalPin, Vec<PrevFFRecord>>)>,
    ...
    pub fn new (input_path : &str)->Self {
        ...
        assert !(
            {
                let first_row_width_height = (mbffg.setting.placement_rows[0].width,
    ...
    fn find_cycle(
        ...
        for
            edge in graph.edges_directed(index, Direction::Incoming) {
    ...
    pub fn build_graph(setting : &Setting) -> Graph<Vertex, Edge> {
        ...
        for
            inst in setting.instances.iter() {
                let gid = graph.add_node(inst.borrow().clone()).index();
                inst.borrow_mut().set_gid(gid);
            }
        for
            net in setting.nets.iter().filter(| net | !net.get_is_clk()) {
                let source = &net.get_pins()[0];
            for
                sink in net.get_pins().iter().skip(1) {
                    graph.add_edge(
    ...
    pub fn get_all_io(&self) -> impl Iterator<Item = &SharedInst>{
        self.graph.node_weights().filter(| x | x.is_io())} pub fn get_all_gate(&self)
        ->impl Iterator<Item = &SharedInst>{
            self.graph.node_weights().filter(| x | x.is_gt())}
    /// Returns an iterator over all flip-flops (FFs) in the graph.
    pub fn get_all_ffs(&self)
        ->impl Iterator<Item = &SharedInst>{
            self.graph.node_weights().filter(| x | x.is_ff())} pub fn get_ffs_sorted_by_timing(&self)
        ->Vec<SharedInst> {
        assert !(
    ...
    pub fn num_io(&self) -> uint{
        self.get_all_io().count().uint()} pub fn num_gate(&self)
        ->uint{
            self.get_all_gate().count().uint()} fn num_bits(&self)
        ->uint{
            self.get_all_ffs().map(| x | x.bits()).sum::<uint>()} pub fn num_ff(&self)
        ->uint{
            self.get_all_ffs().count().uint()} pub fn num_nets(&self)
        ->uint{
            self.setting.nets.len().uint()} pub fn num_clock_nets(&self)
        ->uint{
            self.setting
                .nets
                .iter()
                ...
                .count()
                .uint()} pub fn incomings_edge_id(&self, index : InstId)
        ->Vec<EdgeIndex>{
            self.graph
                .edges_directed(NodeIndex::new (index), Direction::Incoming)
                .map(| e | e.id())
                .collect()} pub fn get_node(&self, index : InstId)
        ->&Vertex{
            &self.graph[NodeIndex::new (index)]} pub fn incomings(&self, index : InstId)
        ->impl Iterator<Item = &Edge>{
            self.graph
                .edges_directed(NodeIndex::new (index), Direction::Incoming)
                .map(| e | e.weight())} pub fn outgoings(&self, index : InstId)
        ->impl Iterator<Item = &Edge>{
            self.graph
                .edges_directed(NodeIndex::new (index), Direction::Outgoing)
                .map(| e | e.weight())} pub fn qpin_delay_loss(&self, qpin : &SharedPhysicalPin)
        ->float {
    ...
    fn get_prev_ffs(&mut self, inst_gid : usize) -> Set<PrevFFRecord> {
        ...
        if inst
            .is_io() {
                ...
                if !self
                    .prev_ffs_cache.contains_key(&source_id) {
                        // If the source does not have a record, create a default one
                        if self
                            .debug_config.debug_create_io_cache {
                                debug !(
        ...
        for
            edge_id in self.incomings_edge_id(inst_gid) {
                let(source, target) = self.graph.edge_weight(edge_id).unwrap().clone();
                let target_id = target.get_id();
                if !self
                    .prev_ffs_cache.contains_key(&target_id) {
                    ...
                    for
                        record in prev_record {
                            ...
                            match current_record.get(&new_record) {
                                ...
                                Some(existing) if new_record.calculate_total_delay(self.displacement_delay()) > existing.calculate_total_delay(self.displacement_delay()) => {
                                    current_record.insert(new_record);
                                }
    ...
    pub fn create_prev_ff_cache(&mut self) {
        if self
            ...
            for
                gid in self.get_all_ff_ids() {
                for
                    edge_id in self.incomings_edge_id(gid) {
                    ...
                    for
                        record in self.prev_ffs_cache.get(&dpin.get_id()).unwrap() {
                            if let
                                Some(ff_q_src) = record.ff_q_src() {
                                    query_map
    ...
    fn correspond_pin(&self, pin : &SharedPhysicalPin) -> SharedPhysicalPin{
        pin.inst().corresponding_pin(pin)} pub fn get_next_ff_dpins(&self, dpin : &SharedPhysicalPin)
        ->&Set<SharedPhysicalPin> {
    ...
    pub fn delay_to_prev_ff_from_pin_dp(&self, edge_id : EdgeIndex) -> PrevFFRecord {
        ...
        let max_record = if src.is_ff() {
            ...
            if cache
                .is_empty() {
                    if self
                        .debug_config.debug_floating_input {
    ...
    pub fn negative_timing_slack_inst(&self, inst : &SharedInst) -> float {
        ...
        self.incomings_edge_id(inst.get_gid())
            .iter()
            .fold(0.0, | acc, edge_id | {acc + self.negative_timing_slack_pin(*edge_id)})
    }
    fn delay_to_prev_ff_from_pin_query(
        &self,
        dpins : Option<&[SharedPhysicalPin]>,
        query_pin : &SharedPhysicalPin, ) -> float {
        let displacement_delay = self.displacement_delay();
        ...
        if let
            ...
            for
                dpin in dpins {
                    if let
                        Some(records) = cache.1.get(&self.correspond_pin(dpin)) {
    ...
    fn update_delay_to_prev_ff_from_pin_query(
        ...
        for
            cor_pin in cor_pins {
                if let
                    Some(records) = cache.1.get(&cor_pin) {
                        let max_delay_record = cal_max_record(records, displacement_delay);
    ...
    fn update_query_cache(&mut self, modified_inst : &SharedInst) {
        ...
        for
            pin in modified_pins_vec.iter() {
                modified_pins.extend(self.get_next_ff_dpins(pin).clone());
            }

        // Closure to avoid repeated Option checks inside loops
        let mut delay_to_prev_ff = | dpin : &SharedPhysicalPin | {self.update_delay_to_prev_ff_from_pin_query(&modified_pins_vec, dpin)};
        for
            query_pin in modified_pins {
    ...
    pub fn negative_slack_effected_from_inst(&self, modified_inst : &SharedInst) -> f64 {
        let effected_insts = self
                                 ...
                                 .map(| x | x.inst())
                                 .collect::<Set<_>>();
        effected_insts.iter().fold(0.0, | delay, inst | {delay + self.negative_timing_slack_inst(&inst)})
    }
    pub fn sta(&mut self) {
        self.create_prev_ff_cache();
        for
            ff in self.get_all_ffs() {
            for
                edge_id in self.incomings_edge_id(ff.get_gid()) {
                    ...
                    if self
                        .debug_config.debug_timing &&delay != ori_delay {
                            info !(
    ...
    pub fn get_free_ffs(&self) -> impl Iterator<Item = &SharedInst>{
        self.graph
            .node_weights()
            .filter(| x | x.is_ff() && !x.get_locked())} pub fn get_legalized_ffs(&self)
        ->impl Iterator<Item = &SharedInst>{
            self.graph
                .node_indices()
                .map(| x | &self.graph[x])
                .filter(| x | x.is_ff() && x.get_legalized())} pub fn get_all_ff_ids(&self)
        ->Vec<usize>{
            self.get_all_ffs().map(| x | x.get_gid()).collect_vec()} pub fn utilization_score(&self)
        ->float {
        ...
        for
            ...
            for
                ...
                if overlap_area
                    > bin_height *bin_width *bin_max_util / 100.0 {
                        overflow_count += 1.0;
                    }
                }
            }
    ...
    pub fn compute_mean_shift_and_plot(&self) {
        ...
        let mean_shifts = self
                              .get_all_ffs()
                              .flat_map(| ff | {ff.dpins().iter().map(| dpin | {
    ...
    pub fn scoring(&mut self, show_specs : bool) -> Score {
        ...
        assert !(
            statistics.total_count == statistics.io_count + statistics.gate_count + statistics.flip_flop_count);
        self.create_prev_ff_cache();
        for
            ff in self.get_all_ffs() {
        ...
        for (key, value) in statistics.lib.iter().sorted_by_key(| x | x.0) {
            ...
            for
                lib_group in value_list.chunks(content.len()) {
    ...
    pub fn output(&self, path : &str) {
        ...
        if let
            Some(parent) = std::path::Path::new (path).parent() {
        ...
        for
            ...
            for
                ...
                for
                    ori_name in original_full_names {
    ...
    pub fn get_lib(&self, lib_name : &str) -> Reference<InstType>{
        ...
        for
            lib_pin in lib.borrow_mut().property().pins.iter() {
    ...
    pub fn bank(&mut self, ffs : Vec<SharedInst>, lib : &Reference<InstType>) -> SharedInst {
        ...
        for
            ff in ffs.iter() {
            for
                dpin in ff.dpins().iter() {
                    self.transfer_edge(dpin, &new_inst_d[d_idx]);
                    d_idx += 1;
                }
            for
                qpin in ff.qpins().iter() {
        ...
        if pin_from
            ...
            let new_edges = if pin_from.is_d_pin() {
                self.incomings(pin_from.get_gid())
                    ...
                    .map(| (src, _) | {(src.get_gid(), pin_to.get_gid(), (src.clone(), pin_to.clone()), )})
                    .collect_vec()
            }
            else if pin_from.is_q_pin() {
                self.outgoings(pin_from.get_gid())
                    .filter(| (src, _) | src.get_id() == pin_from.get_id())
                    .map(| (_, tgt) | {(pin_to.get_gid(), tgt.get_gid(), (pin_to.clone(), tgt.clone()), )})
                    .collect_vec()
            }
            else {
                Vec::new ()};
    ...
    fn clock_nets(&self) -> impl Iterator<Item = &SharedNet>{
        self.setting.nets.iter().filter(| x | x.borrow().is_clk)} pub fn get_clock_groups(&self)
        ->Vec<Vec<SharedPhysicalPin>> {
        let clock_nets = self.clock_nets();
        clock_nets
            .map(| x | {x.clock_pins().into_iter().filter(| x | !x.inst().borrow().locked).collect_vec()})
            .collect_vec()
    }
...
#[derive(PartialEq)]
        ...
        impl Dominate for ParetoElement {
            ...
            fn dominate(&self, x : &Self) -> bool {
                (self != x) && (self.power <= x.power && self.area <= x.area) && (self.width <= x.width && self.height <= x.height)
        ...
        result.sort_by_key(| x | {(x.borrow().ff_ref().bits, OrderedFloat(x.borrow().ff_ref().evaluate_power_area_ratio(self)), )});
        for
            r in 0..result.len() {
                self.library_anchor
    ...
    pub fn unique_library_bit_widths(&self) -> Set<uint>{
        self.retrieve_ff_libraries()
            ...
            .collect()} pub fn find_best_library_by_bit_count(&self, bits : uint)
        ->Reference<InstType>{
            self.pareto_library
                .iter()
                ...
                .expect(&format !(
                    ...
                    self.pareto_library
                        ...
                        .collect_vec()))
                .clone()} pub fn find_all_best_library(&self)
        ->Vec<Reference<InstType>>{
        ...
        for
            ...
            for
                j in 0..row.num_cols {
                    ...
                    if bbox
                        [1][0] > die_width || bbox[1][1] > die_height {
    ...
    pub fn evaluate_placement_resources_from_bits_gurobi(
        &self,
        lib : &Reference<InstType>, ) -> Vec<(f64, f64)> {
        ...
        for
            i in(0..map_shape.0).step_by(STEP_SIZE_X) {
                let range_x = (i..min(i + STEP_SIZE_X, map_shape.0)).collect_vec();
            for
                j in(0..map_shape.1).step_by(STEP_SIZE_Y) {
                    let range_y = (j..min(j + STEP_SIZE_Y, map_shape.1)).collect_vec();
                    let sub_map = map
                                      .fancy_index(&range_x)
                                      .into_iter()
                                      .map(| x | x.fancy_index(&range_y).iter().map(| x | x.is_covered).collect_vec())
                                      .collect_vec();
    ...
    pub fn evaluate_placement_resources_from_bits(
        ...
        if self
            .debug_config.visualize_placement_resources {
    ...
    pub fn generate_occupancy_map(
        ...
        split : i32, ) -> (Vec<Vec<bool>>, Vec<Vec<(float, float)>>) {
        let mut rtree = Rtree::new ();
        if include_ff
            .is_some() {
        ...
        for
            i in 0..self.setting.placement_rows.len() {
    ...
    pub fn group_clock_instances_by_kmeans(
        &mut self,
        clock_pins_groups : Vec<Vec<SharedInst>>, ) -> Vec<(Vec<SharedInst>, f64)> {
    ...
    pub fn merging_kmeans(&mut self) {
        ...
        while
            !clustered_instances_with_distance.is_empty() {
    ...
    fn placement_rows(&self) -> &Vec<PlacementRows>{
        &self.setting.placement_rows} pub fn evaluate_placement_resource(&self,
        ...
        for
            ...
            for
                ...
                for
                    lib in lib_candidates.iter() {
                        let coverage = lib.borrow().ff_ref().grid_coverage(&placement_row);
                        if coverage
                            .0 <= pcell_shape.0 && coverage.1 <= pcell_shape.1 {
                                    ...
                                    let placement_infos = k
                                                              ...
                                                              .map(| (i, x) | {
                                                                  let positions = x
                                                                                      .positions
                                                                                      .iter()
                                                                                      .map(| x | {placement_rows[x.first.usize() + index.0.usize()].get_position(x.second + index.1.i32())})
                                                                                      .collect_vec();
                                                                  PlacementInfo {
                                                                  bits:
                                                                      bits[i],
    ...
    fn error_message(&self, message : String) -> String{
        format !("{} {}", "[ERR]".bright_red(), message)} pub fn is_on_site(&self, pos : (float, float))
        ->bool {
        let(x, y) = pos;
        let mut found = false;
        for
            row in self.setting.placement_rows.iter() {
                if x
                    >= row.x
    ...
    pub fn debank_all_multibit_ffs(&mut self)
        ...
        let mut debanked = Vec::new ();
        for
            ff in self.get_all_ffs().cloned().collect_vec() {
                if ff
                    .bits() > 1 {
    ...
    pub fn displacement_delay(&self) -> float{
        self.setting.displacement_delay} pub fn timing_weight(&self)
        ->float{
            self.setting.alpha} pub fn power_weight(&self)
        ->float{
            self.setting.beta} pub fn area_weight(&self)
        ->float{
            self.setting.gamma} pub fn utilization_weight(&self)
        ->float{
            self.setting.lambda}

    fn group_by_kmeans(&self, ffs : &[SharedInst])
        ->Vec<Vec<SharedInst>> {
    ...
    fn partition_and_optimize_groups(
        ...
        search_number : usize, ) -> Vec<Vec<SharedInst>> {
        ...
        for (group_index, group) in original_groups.iter().enumerate() {
            ...
            for
                instance in group.iter() {
                    let instance_gid = instance.get_gid();
                    if previously_grouped_ids
                        .contains(&instance_gid) {
                            continue;
                        }
                    let mut candidate_group = vec ![];
                    while
                        !rtree.is_empty() && candidate_group.len() < search_number {
                    ...
                    if rtree
                        .is_empty() {
                            ...
                            if self
                                .debug_config.debug_banking_utility {
                                    debug !(
                    ...
                    for
                        partition in combo {
                            ...
                            let partition_utility = if combo_idx == 0 {
                                                                        0.0} else {self.evaluate_utility(&partition_ref, &mut uncovered_place_locator)};
                            valid_mask.push(partition_utility >= 0.0);
                            if partition_utility
                                >= 0.0 {
                    ...
                    if utility
                        > best_utility {
                            ...
                            best_combination = (candidate_index,
                                                combo_idx,
                    ...
                    }
                {
                    ...
                    for
                        instance in candidate_group_set.iter() {
    ...
    fn evaluate_utility(
        ...
        uncovered_place_locator : &mut UncoveredPlaceLocator, ) -> float {
        ...
        let ori_timing_score = self.query_negative_slack_effected_from_inst(instance_group, false) * self.timing_weight();
        ...
        let utility = if let Some(nearest_uncovered_pos) =
            ...
            if self
                .debug_config.debug_nearest_pos {
                    debug !(
    ...
    fn get_group_pa_score(&self, instance_group : &[&SharedInst]) -> float{
        instance_group
            .iter()
            .map(| x | {x.get_lib().borrow().ff_ref().evaluate_power_area_score(self)})
            .sum()}
    ...
    pub fn merge(&mut self, physical_pin_group : &[SharedInst]) {
        ...
        for
            optimized_group in optimized_partitioned_clusters.into_iter() {
...
for (bit_width, group_count)

    in bits_occurrences.iter() {
...
pub fn gurobi_merge(&mut self, clustered_instances : &[SharedInst]) {
    ...
    let item_to_index_map : Dict<_, Vec<_>> = knn_results_for_ffs
                                                  .iter()
                                                  .flat_map(| (i, ff_list) | {ff_list.iter().enumerate().map(move | (j, ff) | (ff.item, (*i, j)))})
...
pub fn get_prev_ff_records(&self, dpin : &SharedPhysicalPin) -> &Set<PrevFFRecord>{
    self.prev_ffs_cache.get(&dpin.get_id()).expect(self.error_message(format !("No records for {}", dpin.full_name())).as_str(), )} pub fn get_prev_ff_records_from_inst(&self, inst : &SharedInst)
    ->Set<&PrevFFRecord>{
        inst.dpins()
            .iter()
            .flat_map(| x | self.get_prev_ff_records(x))
            .collect()} pub fn get_prev_ff_records_count(&self, inst : &SharedInst)
    ->usize {
    inst.dpins()
        .iter()
...
impl MBFFG {
    ...
    pub fn get_pin_util(&self, name : &str) -> SharedPhysicalPin {
        ...
        if self
            .current_insts.contains_key(inst_name) {
                return self
                    .get_ff(inst_name)
                    .get_pins()
    ...
    fn retrieve_prev_ffs_mindmap(
        ...
        while
            buffer.len() > 0 {
                ...
                if let
                    Some(stop_at_level) = stop_at_level {
                        if level
                            > stop_at_level {
                                return;
                            }
    ...
    pub fn visualize_mindmap(
        ...
        for
            edge_id in self.incomings_edge_id(inst.get_gid()) {
    ...
    pub fn get_inst(&self, name : &str) -> &SharedInst{
        &self.current_insts[&name.to_string()]} pub fn visualize_timing(&mut self) {
        self.create_prev_ff_cache();
        let timing = self
                         .get_all_ffs()
